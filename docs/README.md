# Project Docs

This folder contains the working documents we reference while building the Phase 1 MVP.

- `docs/AGENTS.md`: Instructions for agentic coding agents operating in this repo.
- `docs/RULES.md`: Scope boundaries, conventions, and decision log for this project.
- `docs/MILESTONES.md`: Milestone plan and definitions of done.
- `docs/ARCHITECTURE.md`: High-level architecture and module boundaries.
- `docs/ENVIRONMENT.md`: Local/staging/prod environment variable strategy.
- `docs/API_CONVENTIONS.md`: REST conventions, error shape, pagination, auth headers.
- `docs/DATA_MODEL.md`: ERD-style notes and ownership rules (tenant scoping).
- `docs/LOGGING_AUDIT.md`: Application logging vs audit logging expectations.

Keep these docs short and practical. Prefer checklists and concrete decisions over prose.
