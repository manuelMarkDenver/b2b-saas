...
| title | String | Short summary shown in the bell panel (e.g. "New order received") |
| body | String | Detail line (e.g. "Order #abc123 for $850 is pending confirmation") |
| entityType | String? | e.g. `order`, `payment`, `sku` — used to build the action link |
| entityId | UUID? | ID of the related entity — click navigates to it |
| isRead | Boolean | `false` by default. Set to `true` when user opens/clicks the notification. |
| createdAt | DateTime | |

**Rules:**
- Written server-side when the triggering event occurs — never from the frontend.
- In-app notifications are always created regardless of external channel preferences.
- Read via `GET /notifications` (scoped to the authenticated user). Unread count returned in the same response for the badge.
- `PATCH /notifications/:id/read` marks one as read. `PATCH /notifications/read-all` marks all read.
- `DELETE /notifications/:id` dismisses (hard-deletes) a notification.
- Notifications older than 90 days are purged on a scheduled job (prevents unbounded table growth).
...
| UserNotificationPreferences | Post-MVP (per-user channel config: email/SMS/Messenger/WhatsApp opt-in + contact details) |
