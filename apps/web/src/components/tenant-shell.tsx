...
import { TenantFeaturesProvider } from '@/lib/tenant-features-context';
...
export function TenantShell({ tenantSlug, tenantName, children }: TenantShellProps) {
  ...
  useEffect(() => {
    const stored = localStorage.getItem('zentral:sidebar:collapsed');
  ...
  function handleToggleCollapse() {
    setSidebarCollapsed((v) => {
      const next = !v;
      localStorage.setItem('zentral:sidebar:collapsed', String(next));
      return next;
    });
  }
    ...
    async function load() {
      ...
      if (membershipsRes.ok) {
        ...
        if (current?.tenant.features) {
          ...
          setDisplayName(current.tenant.name);
